...
sh "mv ${WORKSPACE}/${BUILD_NUMBER}/${pomvalues.artifactId}-${pomvalues.version}.${pomvalues.packaging} ./${pomvalues.artifactId}.${pomvalues.packaging}"
  sh "docker build . -t ${duser}/${imagename}:${BUILD_NUMBER} --build-arg \"database_url=${dburl} database_user=${dbuser} database_password=${dbpassword} artifact=${pomvalues.artifactId}.${pomvalues.packaging}\""
